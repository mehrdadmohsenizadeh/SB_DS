def show_camera_image(
    ...
    """
    Displays a camera image and overlays bounding boxes for detected objects on a matplotlib subplot.

    Parameters:
    - camera_image (CameraImage): The camera image object, which must contain an encoded image and the camera's name.
    - camera_labels (list of CameraLabel): A list of camera label objects for detected objects within the camera image.
    - layout (tuple): The layout parameters for plt.subplot (e.g., nrows, ncols, index).
    - cmap (str, optional): The colormap for displaying the image. If None, the image is shown in its original colors.

    Raises:
    - ValueError: If the camera_image or camera_labels are empty or if no labels match the camera image's name.

    Note:
    - This function modifies the current matplotlib figure by adding a subplot with the specified image and annotations.
    - CameraImage and CameraLabel are expected to follow a specific structure, typically aligned with dataset formats such as Waymo's.
    """
    if not camera_image or not camera_labels:
        raise ValueError("Camera image and camera labels must not be empty.")

    ax                    = plt.subplot(*layout)
    matching_labels_found = False

    # Draw the camera labels
    for camera_label in camera_labels:

        # Check if labels correspond to the current camera
        if camera_label.name == camera_image.name:
            matching_labels_found = True
    ...
    if not matching_labels_found:
        raise ValueError(f"No labels found that match the camera image's name: {camera_image.name}")
